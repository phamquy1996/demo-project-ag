...
export default function VpsPolicy() {
  ...
  const [tempException, setTempException] = useState({ value: 0, label: 'Chính sách VPS' })
  ...
  const { data: searchResult = { content: [], totalElements: 0, totalPages: 0 }, isLoading: isSearchLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: () => policyService.search({
      ...
      exception: searchParams.exception,
      page: currentPage - 1,
      size: itemsPerPage
    }),
    staleTime: 10000
  ...
  // Trigger search
  const handleSearch = () => {
    // Validate ngày đến ngày phải luôn lớn hơn hoặc bằng từ ngày
    ...
    setSearchParams({
      ...
      approveStatus: filterApproveStatus !== null && filterApproveStatus !== undefined ? filterApproveStatus.value : '',
      activeStatus: filterActiveStatus !== null && filterActiveStatus !== undefined ? filterActiveStatus.value : '',
      fromDate: tempFromDate ? formatDateToYMD(tempFromDate) : '2026-01-01',
      toDate: tempToDate ? formatDateToYMD(tempToDate) : '2026-12-31',
      accountId: tempAccountId.trim(),
      accountName: tempAccountName.trim(),
      exception: tempException ? tempException.value : 0
    })
    
    // Gọi trực tiếp hàm refetch của React Query để ép tải lại dữ liệu mới từ backend
    refetch()
  }
  ...
  const handleClearFilters = () => {
    ...
    setTempAccountId('')
    setTempAccountName('')
    setTempException({ value: 0, label: 'Chính sách VPS' })
    setSearchParams({
      operatorCode: '',
      approveStatus: '',
      activeStatus: '',
  ...
  const vpsColumns = [
    ...
    },
    {
      ...
      render: (p, classes) => {
        ...
        if (scope === 0) return <span className={classes.badge} style={{ backgroundColor: '#e0f2fe', color: '#0369a1' }}>Toàn bộ VPS</span>
        if (scope === 1) return <span className={classes.badge} style={{ backgroundColor: '#f3e8ff', color: '#6b21a8' }}>Theo KH</span>
        return <span className={classes.badge} style={{ backgroundColor: '#cbd5e1', color: '#475569' }}>Toàn bộ VPS</span>
      },
      width: '1.2fr'
    },
    {
      header: 'Tài khoản KH',
      render: (p) => p.accountId || '-',
      width: '1.2fr'
    },
    {
    ...
    { 
      header: 'OTP', 
      render: (p, classes) => (
        <span className={`${classes.badge} ${Number(p.otpStatus) === 1 ? classes.badgeSuccess : classes.badgeDanger}`}>
          {Number(p.otpStatus) === 1 ? 'Có' : 'Không'}
        </span>
      ),
      width: '0.6fr'
    },
    { 
      ...
      render: (p) => {
        if (Number(p.otpStatus) !== 1) return '-'
    ...
    { 
      header: 'BIO', 
      render: (p, classes) => (
        <span className={`${classes.badge} ${Number(p.bioStatus) === 1 ? classes.badgeSuccess : classes.badgeDanger}`}>
          {Number(p.bioStatus) === 1 ? 'Có' : 'Không'}
        </span>
      ),
      width: '0.6fr'
    },
    { 
      ...
      render: (p) => {
        if (Number(p.bioStatus) !== 1) return '-'
      ...
      <div className={styles.filterCard} onKeyDown={handleKeyDown}>
        <div className={styles.filterGrid}>
          ...
          <div className={styles.filterField} style={{ maxWidth: '180px', flex: 1 }}>
            ...
            <div className={styles.inputWithIcon} style={{ width: '100%' }}>
              <input 
                ...
                style={{ paddingLeft: '10px', height: '34px', border: '1px solid var(--input-border)', borderRadius: '8px', width: '100%', fontSize: '13px', backgroundColor: 'var(--input-bg)', color: 'var(--text-primary)' }}
              />
            </div>
          </div>
          ...
          <div className={styles.filterField} style={{ maxWidth: '180px', flex: 1 }}>
            ...
            <Select
              options={[
                { value: 0, label: 'Chính sách VPS' },
                { value: 1, label: 'Ngoại lệ' }
              ]}
              value={tempException}
              onChange={(opt) => setTempException(opt)}
          ...
          <div className={styles.filterActions}>
            ...
            <button className={styles.btnSecondary} onClick={() => refetch()} disabled={isTableLoading} title="Tải lại dữ liệu">
              <FontAwesomeIcon icon={faRotate} spin={isSearchLoading} />
              Tải lại
            </button>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 2fr', gap: '12px', marginBottom: '16px', textAlign: 'left' }}>
                  <div className={styles.modalFormGroup} style={{ marginBottom: 0 }}>
                    ...
                    <Select
                      options={[
                        { value: 1, label: 'Có (ON)' },
                        { value: 0, label: 'Không (OFF)' }
                      ]}
                      value={{ value: formOtpStatus, label: formOtpStatus === 1 ? 'Có (ON)' : 'Không (OFF)' }}
                  ...
                  <div className={styles.modalFormGroup} style={{ marginBottom: 0 }}>
                    ...
                    <input 
                      ...
                      disabled={formOtpStatus === 0}
                      style={formOtpStatus === 0 ? { backgroundColor: '#f1f5f9', cursor: 'not-allowed' } : {}}
                    />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 2fr', gap: '12px', textAlign: 'left' }}>
                  <div className={styles.modalFormGroup} style={{ marginBottom: 0 }}>
                    ...
                    <Select
                      ...
                      value={{ value: formBioStatus, label: formBioStatus === 1 ? 'Có (ON)' : 'Không (OFF)' }}
                  ...
                  <div className={styles.modalFormGroup} style={{ marginBottom: 0 }}>
                    ...
                    <input 
                      ...
                      disabled={formBioStatus === 0}
                      style={formBioStatus === 0 ? { backgroundColor: '#f1f5f9', cursor: 'not-allowed' } : {}}
                    />
